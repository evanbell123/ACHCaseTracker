(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('achCaseTrackingApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-achCaseTrackingApp-error');
                var entityKey = httpResponse.headers('X-achCaseTrackingApp-params');
                if (errorHeader) {
